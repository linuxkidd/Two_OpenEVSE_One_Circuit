...
3. In the ShellyEM, Enable MQTT and configure the MQTT Broker under: *Internet & Security* -> *Advanced - Developer Settings*
...
5. On the Primary OpenEVSE, set the charge current limit based on the following formula:
   ...
   ```
   This is set at: *Settings* -> *EVSE* -> *MAX CURRENT*
6. On the Secondary OpenEVSE:
	* Set the *MAX CURRENT* value to the same as that calculated and set on the Primary OpenEVSE
	* Enable MQTT and configure the MQTT Broker under: *Settings* -> *MQTT*
	* Configure the Shaper under: *Settings* -> *Shaper*
